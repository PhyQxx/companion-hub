...
### 多终端主动感知
电脑与浏览器扩展可注册为 Aria 的感知终端。你可以在 Web 对话中说"帮我看看电脑上的网页"，她会让对应设备执行受控命令并带回结果。Home Assistant、MQTT 遥测、M3B 认知调度闭环与主动多终端输出（Web/桌面通知/语音）已接入；屏幕感知 v1 支持周期性截屏与视觉分析并沉淀为记忆和主动话题。手机 PWA 与首个 ESP32 存在传感器仍在后续计划中。
