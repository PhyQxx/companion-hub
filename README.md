...
### 人格与形象
你可以为她设定完整的人格（Persona），从说话风格到行为边界；也可以选择或自定义她的视觉形象。人格与形象解耦，换形象不会丢失记忆和关系。
...
### 多终端主动感知
手机、电脑、浏览器扩展均可注册为 Aria 的感知终端。你可以从任何设备对她说"帮我看看电脑上的网页"，她会让对应的设备执行命令并带回结果。未来还将接入 MQTT 传感器与受控主动感知。
...
内置多套主题，支持深浅色跟随系统、定时切换与实时预览；角色设定包含多角度立绘、表情差分和配色方案。
