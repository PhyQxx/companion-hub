...
远端 Schema 是不可信数据：保守建模见 modeling.py，不支持的结构一律跳过
并给出原因，绝不动态执行远端逻辑。所有 MCP 写动作固定 A2（每次确认）、
不可逆、L1 上限、回执验证——本地策略权威，远端 readOnlyHint 只作目录参考。
"""
...
from .modeling import build_flat_arguments_model, wrap_nested
...
def build_arguments_model(schema: Any) -> type[BaseModel] | None:
    """动作参数模型：{"arguments": {远端标量属性}} 嵌套形状。"""
    flat = build_flat_arguments_model(schema)
    return wrap_nested(flat) if flat is not None else None
