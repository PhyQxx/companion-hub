...
function renderCurrent(current) {
  ...
  $("#default-emotion").textContent = persona.default_emotion;
...
async function publish(version) { if (!confirm(`发布 Persona 草稿 v${version}？`)) return; try { await request(`/versions/${version}/publish`, {method:"POST"}); toast(`Persona v${version} 已发布`); await load(); } catch (error) { toast(error.message, true); } }
async function rollback(version) { if (!confirm(`回滚到 Persona v${version}？`)) return; try { const result = await request(`/versions/${version}/rollback`, {method:"POST"}); toast(`已创建回滚版本 v${result.version}`); await load(); } catch (error) { toast(error.message, true); } }
