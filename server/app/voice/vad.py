...
silero-vad（M2 后续批次）落地前先以 stdlib audioop 实现确定性断句；
接口按 VadEvent 状态迁移设计，替换实现不需要改动调用侧。
"""
...
class EnergyVad:
    ...
    def is_voiced(self, pcm: bytes) -> bool:
        ...
        return audioop.rms(pcm, 2) >= self._threshold

    def _step(self, frame: bytes) -> VadEvent | None:
        voiced = audioop.rms(frame, 2) >= self._threshold
        if not self._speaking:
