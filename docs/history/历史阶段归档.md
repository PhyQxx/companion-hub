...
每日记录与期末闸门结果由使用者持有，回填时按下表登记（示例行说明格式，
不是事实记录；回填后删除本句）：

| 项目 | 位置（外部路径/文件名） | 覆盖日期 | 结果 |
|---|---|---|---|
| 每日文字使用记录（14 天） | _待填_（如 `~/aria-p5-log/day01.md`） | _待填_ | _待填_ |
| 期末全量闸门（ruff/mypy/pytest/前端构建） | _待填_ | _待填_ | _待填_ |
| 使用期检查单结论（§5 各项） | _待填_ | _待填_ | _待填_ |
