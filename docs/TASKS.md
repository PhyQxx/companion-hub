...
| P6 Batch D Live2D/桌宠 | Tauri 代码底座完成 | 透明置顶窗口、拖拽、穿透、位置恢复和 Hub 同源形象舞台已接通；M2 与真机性能仍是发布门槛 |
...
- [ ] **当前功能：Tauri 透明桌宠。** 用户决定先推进代码底座；透明置顶窗口、拖拽、鼠标穿透、位置恢复、托盘解锁和 Hub 同源形象舞台已完成，下一步接正式语音/情绪事件通道并做 macOS 多屏与性能真机验收。M2 仍作为发布门槛，不阻塞代码开发。
...
- [ ] P6 Batch D Tauri 桌宠：透明置顶窗口、点击穿透、拖拽、位置恢复及 Hub 同源舞台代码已完成；待接正式语音/情绪事件通道，并完成 macOS 多屏、CPU/内存与长时间运行验收。M2 首音频 P90 ≤1.8s、打断 P90 ≤300ms 保留为发布门槛。
...
- [x] Tauri 透明桌宠代码底座：新增独立透明置顶窗口与 Vite 多页面入口，支持主窗口显隐/鼠标穿透控制、拖拽、物理坐标恢复和托盘强制恢复交互；形象舞台由 Hub 同源提供，复用当前 Live2D/静态/抽象形象且不放宽 `/api/v1/avatar-user-assets/` 安全白名单。M2 保留为发布验收门槛，下一步补正式语音/情绪事件通道及 macOS 多屏/性能真机验收。
