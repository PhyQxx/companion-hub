...
- [x] **MCP-D 写工具接入 Action Plan（2026-09-11 代码闭环，待真实写 Server 验收）。** 目录刷新回调把白名单写工具动态同步进 Action Registry：固定 A2 每次确认、不可逆、L1 上限、超时取 `call_timeout_seconds`，目录消失即移除；远端 Schema 只接受保守标量子集（拒绝 anyOf/$ref/array 等），生成 `{"arguments": {...}}` 嵌套模型保证执行器层可复验。唯一写入口 `mcp_tool_call`（`runs_local=False`、max L1、参数 16KB 上限）只注册进计划执行器，聊天挂载恒关；`McpManager.call` 保持写硬拦截，`call_write` 仅供确认后的 Runner。Runner 对 MCP 步骤固定 L1 上下文（L2 内容不外发），RECEIPT 验证器 `mcp.call_receipt` 只留 server/tool/ok 回执证据。回归 7 项新增用例；全量 832 通过、mypy 零错误。审计沿用 action_plan 执行台账。
