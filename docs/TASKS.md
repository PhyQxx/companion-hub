...
- [x] **MCP-D 写工具接入 Action Plan（2026-09-11，含真实链验收）。** 目录刷新回调把白名单写工具动态同步进 Action Registry：固定 A2 每次确认、不可逆、L1 上限、超时取 `call_timeout_seconds`，目录消失即移除；远端 Schema 只接受保守标量子集（拒绝 anyOf/$ref/array 等），生成 `{"arguments": {...}}` 嵌套模型保证执行器层可复验。唯一写入口 `mcp_tool_call`（`runs_local=False`、max L1、参数 16KB 上限）只注册进计划执行器，聊天挂载恒关；`McpManager.call` 保持写硬拦截，`call_write` 仅供确认后的 Runner。Runner 对 MCP 步骤固定 L1 上下文（L2 内容不外发），RECEIPT 验证器 `mcp.call_receipt` 只留 server/tool/ok 回执证据。真实链验收通过（`server/scripts/local_mcp_stub.py` 官方 SDK 回环 Server：连接→同步→A2 确认→执行 completed→回执 VERIFIED→写入落盘→跨进程回读→审计台账）；外部写 Server 接入时按同链路复核。
- [x] **MCP-C2 只读工具按意图动态挂载（2026-09-11，含实机验证）。** `McpChatToolProvider` 每轮按词法相关性（拉丁 token + 中文 2-gram，阈值 0.12）从目录挑只读工具，最多 `mcp.max_tools_per_turn` 个注入本轮工具定义；无关文本零挂载（工具数增长不推高普通对话 Token），L2/L3 回合与总开关关闭均不挂。远端自由文本绝不进提示词：给模型的是 Hub 稳定文案 + 经 `modeling.py` 保守重建的参数 Schema（C2/D 共享构建器）；执行走独立临时注册表，handler `runs_local=False` + max L1 由 Egress 兜底。验收 4 项新增用例 + 真实 Context7 实机（相关查询选中两工具、无关/L2 零挂载、真实调用返回库 ID）；全量 836 通过、mypy 261 文件零错误。
...
- [x] `BRIEF-01` 每日智能简报（代码闭环，待真实验收）：`daily_brief` 表（`0028_daily_brief` 迁移，`(user_id, brief_date)` 唯一保证每日 exactly-once）+ `DailyBriefService`/`DailyBriefScheduler`。事实采集确定性：天气（高德实时+当日预报，默认城市取 `config.tools.query.default_city` 或 `ARIA_BRIEF_CITY`，失败仅少一条事实）、当日会触发的时间任务（`task:{id}`）、当日到期/已过期承诺（`goal:{id}`，过期标注）；正文用确定性模板拼装而非 LLM——任务/目标标题是用户文本，进模型提示词既有注入面又不可溯源。每条事实持久化来源引用满足"结论可查看来源"；无任务与承诺时正文一行短句。本地时区（`ARIA_DEFAULT_TIMEZONE`）到 `ARIA_BRIEF_TIME`（默认 08:00）后为每个活跃用户投递一次，经 `ProactiveDeliveryService` 走 Web/桌面/语音通道；用户 API `GET /api/v1/briefs(/latest)` 与 `POST /api/v1/briefs/generate`（幂等预览不投递）。2026-09-16 **当日日程事实接入**（J4 日历含外部镜像已就绪，解除原"日程待接入后扩展"暂缓）：`今日日程（N）`区块按时序列出当日事件（本地/`[caldav]`/`[google]` 来源标注 + 地点 + 全天归并），每条带 `calendar:{id}` 来源引用；同日**通勤建议接入**（解除原"通勤待接入后扩展"暂缓）：`出行建议`行给出当日首个带地点日程的建议出发时刻（CommuteService 只读计算、不建提醒、失败静默降级、非当日日程不出现在简报）。至此原暂缓三项仅剩家庭状态（待传感器真源）。
...
- [x] `SAT-01` Hub 侧房间终端协议（代码闭环，设备端/真机待验收）：新增 `satellite.audio.start/chunk/end` 上行，固定 PCM16/16k/mono，连续分片、4 MiB 上限、结束字节数与 SHA-256 校验，断线/重连/乱序/越权均清理在途话语。完整 PCM 复用现有 ASR → ChatService 流式回合 → TTS 链，下行控制与 base64 音频分片仍走设备签名帧；每台卫星复用独立连续会话，L2 强制本地 ASR/TTS。2026-09-16 设备端参考实现落地：`integrations/satellite_client`（设备凭据鉴权连接、唤醒仲裁上报、能量 VAD + hangover 话语切分、PCM 分片上传含 SHA-256、TTS 分片播放（PCM 直播/MP3 经 --player）、免唤醒窗口内自动续录、speaking 中人声 barge-in、`t` 键跨房间接管；唤醒词可选 openwakeword，缺省回车触发）。剩余验收：真机跑真实全链。
...
- [x] `CAL-01` 日历（代码闭环，待真实验收）：v1 以 Hub 本地库为日历真源（`calendar_event` 表，`0030_calendar` 迁移同时给 `task_item` 加 `source_ref` 关联列）；外部 CalDAV/Google 后续按同契约接入。`CalendarStore` 半开区间重叠查询（首尾相接不算冲突）；`CalendarService` 提供 `preview`（纯读：规范化展示时间/参与人/目标日历 + 冲突列表，不落库，对应验收「写入前展示」）与显式 create/patch/cancel；会前提醒复用 TASK-01 调度底座（`source_ref=calendar:{id}`，默认提前 10 分钟，事件太近则尽快提醒；改期撤旧建新、取消撤旧，不引入新调度器）。API `/api/v1/calendar/events(/preview)` 全部走聊天会话鉴权；聊天端经 `calendar_create` 受控工具接入（两段式：先预览复述、用户确认后落库，时间冲突服务端硬拦），不开放模型对底层 API 的直呼。2026-09-16 CalDAV 只读镜像同步 v1 落地：`integrations.calendar.caldav` 配置（secret 双模式 + Admin 脱敏）、PROPFIND 发现 + REPORT 时间窗拉取、周期/EXDATE/override 客户端展开、`source_ref=caldav:{uid}#{occurrence}` 镜像 upsert（etag 跳过、远端删除联动取消、镜像不建本地提醒）、CalDavSyncScheduler（`ARIA_CALDAV_SYNC_INTERVAL` 默认 900s）与 `POST /api/v1/calendar/caldav/sync`。2026-09-16 Google 日历接入：外部镜像引擎抽为共享 `CalendarMirrorService`（CalDAV/Google 共用 upsert/etag/删除联动）；`integrations.calendar.google` 配置（client_id + secret 双模式 + Admin 脱敏）；OAuth 流程 `GET /api/v1/calendar/google/authorize`（HMAC 签名 state，10 分钟有效）→ `/google/callback` 换刷新令牌落库（`calendar_oauth_token` 表，0044 迁移，按用户+提供方唯一，不入配置/日志）→ `POST /api/v1/calendar/google/sync`（Calendar API v3 `singleEvents=true` 服务端展开周期，周期实例按 recurringEventId+originalStart 字面量锚定稳定 ref）+ 900s 调度器（`ARIA_GOOGLE_SYNC_INTERVAL`）+ `DELETE /api/v1/calendar/google/token` 撤销。剩余：真实 CalDAV 服务器联调验收；Google OAuth 真实凭据走完整授权流。
- [x] `TODO-01` 单一任务真源（对接 pnkx，2026-09-03 真机联调通过）：**pnkx 待办为唯一真源**，Hub `task_item` 作本地镜像（`0031_todo_sync` 迁移加 `external_updated_at/priority/group_label` 三列）。鉴权按用户决策采用**内部集成令牌**：pnkx 侧新增 `IntegrationTokenFilter`（`X-Integration-Token` 头常量时间比对，绑定 `pnkx.integration.userId` 指定身份，未配置完全不生效），Aria 侧 `ARIA_PNKX_BASE_URL` + `ARIA_PNKX_TOKEN` 两个 env 齐备才启用——无密码存储、无会话过期。同步引擎 `TodoSyncService`：全量分页拉取（跳过子任务）→ 按 `source_ref=pnkx:{id}` upsert 镜像、`external_updated_at` 变更才更新、远端删除→本地取消；本地完成→推送 `status=1`；Aria 手建任务经 `clientUuid=aria:{task_id}` 推送 pnkx，推送后崩溃未回填时按 clientUuid 从拉取快照认领，绝不重复创建。镜像 `next_fire_at` 恒为 NULL 不产生本地提醒（pnkx 自带 Quartz 提醒，避免双端重复打扰）；简报/回顾自动包含镜像任务。调度器默认 300s（`ARIA_TODO_SYNC_INTERVAL`），API `POST /api/v1/todo/sync` 手动触发返回统计。延期/优先级反向推送已于 2026-09-16 落地：`PATCH /api/v1/tasks/{id}`（priority/defer_until）、镜像 `pending_priority`+`next_fire_at` 显式标记、同步循环先推字段变更再拉取、`claim_due` 永不认领 pnkx 镜像（双端通知红线）；真实 pnkx 联调待验收。
- [ ] `MAIL-01` 邮件助手（v1 主体与 FIX-01 已实现，待更新后的真实体验验收）：SMTP/IMAP 配置、收件摘要、QQ 邮箱历史联调已通过；发送现改为 `mail_send` 只准备预览，用户在 Chat 完整邮件卡片点击确认，经鉴权 API 核对身份/有效期/内容摘要后发送，模型 `confirmed=true` 不再允许发送。临时预览重启失效，结果未知不得自动重试。2026-09-16 增强：预览已随 FIX-01 持久化底座落库（重启/跨 worker 存活）；UID 化收件箱（unread 标记/unread_only 过滤/`mail_mark` 已读未读工具）；附件上传 API（MIME 白名单/10MiB/12 件待发上限）→ 预览引用 → 确认发送消费。2026-09-16 同日补文件夹归类：`mail_folders`/`mail_move` 工具（IMAP LIST + UID MOVE，移动前先经 LIST 校验目标存在）与 `mail_read`/`mail_mark` 的 folder 参数。剩余：新确认流程真实模型/邮箱验收。
...
- [x] 局域网/VPN 安全接入（部署配置完成，实际组网验收待做）：compose 新增 `vpn` profile 的 Tailscale 子网路由边车（`TAILSCALE_AUTHKEY` + 固定 `ARIA_SUBNET`），边界/步骤/验收清单见 [07-安全与管理后台](./07-安全与管理后台.md) 的「安全远程接入 NET-01」章节。
...
- [ ] M2：低延迟语音专用 LLM 端点评估按用户决定暂时忽略；ASR 已先用 VAD hangover 预取覆盖部分整段识别等待（本地 ASR 已是 beam=1/无 VAD filter，机械优化点用尽，进一步下探需流式 ASR 或低延迟端点）。2026-09-16 补上 `GET/POST /api/v1/admin/voice/latency(/reset)` 判卷报告与统计窗口重置入口；下一步正式计样 20 个完整回合 + 20 个打断。
...
- [x] 2026-09-16 `附件上传 UI + 协议一致性测试 + env 文档`：聊天输入区上方新增**附件条**（📎 上传按钮 + 待发附件 chip + 移除；仅 L1 显示，10MiB 前端预检，multipart 上传 `POST /api/v1/mail/attachments`，与待发邮件预览、`mail_attachments` 模型工具构成完整闭环：上传 → 告诉 Aria 发给谁 → 预览卡片确认）；新增 test_satellite_client_protocol **4 项**——按路径加载 `integrations/satellite_client` 与 Hub `sign_device_frame` 交叉验证签名一致性、分片尺寸对齐帧 schema 上限、关键帧字段过 pydantic 模型校验、规范化 JSON 规则一致——任何一侧改协议这组测试即失败，防止设备端与 Hub 漂移；`.env.example` 补齐 `ARIA_CALDAV_SYNC_INTERVAL/ARIA_GOOGLE_SYNC_INTERVAL/TAILSCALE_AUTHKEY/ARIA_SUBNET/ARIA_ENABLE_DEV_ENDPOINTS` 说明。Chat typecheck/build、全量非 soak 0 失败、Ruff、mypy 通过。

- [x] 2026-09-16 `安全修复 + 附件闭环 + Admin 日历卡片`：**dev 事件注入接口加鉴权**（`/api/v1/dev/events` 此前完全无鉴权且 compose 默认开启——能触达 Hub 端口即可伪造感知事件触发场景计划/污染记忆；现在配置了 `ARIA_ADMIN_TOKEN` 即强制 Bearer 校验，纯本地无令牌开发保持放行；compose 默认改 `false`，新增 1 项回归）；`mail_attachments` 工具（列出待发附件 id/文件名，模型可引用填进 `mail_send.attachment_ids`，附件流闭环；+1 回归，邮件合计 21）；Admin「HA 实体授权」页新增**外部日历卡片**（CalDAV 地址/凭据/日历名单 + Google client_id/密钥/回调/日历 ID 配置、「立即同步」按钮走新 Admin 端点 `POST /api/v1/admin/config/integrations/calendar/{provider}/sync`、Google 授权页直达链接；配置经既有 PUT current 保存即热生效）。Admin typecheck/build、全量非 soak 0 失败、Ruff、严格 mypy（278 files）通过。

- [x] 2026-09-16 `开发批次二（SAT 设备端 / 邮件归类 / Google 日历）`：SAT-01~03 设备端参考客户端 `integrations/satellite_client`（鉴权连接、唤醒仲裁、能量 VAD 话语切分、签名分片上传、TTS 播放与 barge-in、免唤醒窗口、跨房间接管，README 含配对/运行/红线说明）；MAIL-01 文件夹归类（`mail_folders`/`mail_move` 工具 + folder 参数贯通）；CAL-01 Google 日历（共享镜像引擎 `CalendarMirrorService` 抽象、OAuth 授权流 + 刷新令牌落库 `0044` 迁移、Calendar API v3 只读镜像同步 + 调度器 + 撤销端点；周期实例 ref 用 originalStart 字面量锚定跨时区稳定）。新增回归：test_calendar_google **8**、test_mail +2（文件夹工具与参数透传，合计 20）。非 soak 全量 pytest 0 失败，Ruff、严格 mypy（278 source files）、Alembic 单 head `0044_google_oauth_token` 通过。

- [x] 2026-09-16 `代码侧收尾批次（真实验收前置）`：DIAG 临时诊断日志清除（f8fc745 逆操作，解耦修复保留）；Admin「提醒与任务」可视化页（`/api/v1/admin/tasks` 分页/筛选/状态计数/取消 + TasksView）；FIX-01/01B 确认预览持久化（`pending_mutation` 表 0040 迁移，`DatabasePendingMutationStore` 跨重启/跨 worker，认领条件更新原子守卫；邮件/日程/流程三端切换并保持前端契约）；TODO-01 反向推送（0041 迁移加 `pending_priority`，PATCH API + 同步推送 + 镜像永不本地触发）；MAIL-01 未读管理（UID SEARCH/FETCH + FLAGS、`mail_mark` 工具、unread_only）与附件发送（`mail_attachment` 表 0042 迁移 + 上传 API + 预览引用 + 确认消费）；CAL-01 CalDAV 只读镜像（0043 迁移加 `source_ref/external_etag`，发现/时间窗/周期展开/镜像 upsert + 调度器 + 手动同步 API）；MEET-01 录音客户端 v1（`integrations/meeting_recorder`，音频即焚）；NET-01 Tailscale 子网路由 compose profile + 文档章节；M2 判卷 `GET/POST /api/v1/admin/voice/latency(/reset)`；修复 test_safety API 用例与后台升级任务的 FakeTime 竞态（后台升级链先停再断言，全量 6/6 稳定）；P5 闸门报告补外部证据索引占位表。全量非 soak **911 通过 / 0 失败**，Ruff、严格 mypy（275 source files）、Chat/Admin typecheck 与 production build、Alembic 单 head `0043_calendar_mirror`、`git diff --check` 全部通过。

- [x] 2026-09-15 `主动投递排障与解耦修复`：复核 DIAG 日志——本地 `/tmp/aria_deliver_diag.log` 全部为 pytest 样本（测试函数名与 SAFE 测试 trigger），线上循环接线验证正常（浏览器感知 51 cycles 零失败、观察持续产出，proactive 各开关开启）；「有观察无投递」当前命中的是设计内门控（`notable=false`、认知判定 ignore、感知策略去重/DND/预算），桌面端「我的 Mac」离线期间屏幕感知按设计降级。代码修复：`ProactiveDeliveryService` 与安全告警/久未活动/任务/承诺/专注/简报/回顾的投递接线从 `if home_assistant_manager` 解耦到 `if runtime_config`（等价重构，消除 HA 构造缺席时整个主动栈静默失效的结构性风险），HA Proactive 引擎保留原条件。mypy 通过、非 soak 全量 877 通过、本地 Hub 热重载健康。DIAG 诊断日志已于 2026-09-16 清除（f8fc745 逆操作）；如再复现「应送达未送达」，按 41a4657 的解耦思路重查主动栈接线。
...
- 2026-09-16 代码侧收尾批次（真实验收前置）：新增/更新回归 **test_admin_tasks 8、test_confirmation_db 7、test_todo_sync +3、test_tasks +2、test_mail 18 全量（含附件/未读 4 新增）、test_calendar_caldav 6、test_admin_voice 3**；全量非 soak pytest **911 通过 / 0 失败 / 2 跳过**；Ruff、严格 mypy（275 source files）、Chat/Admin typecheck 与 production build、Alembic SQLite 空库单 head `0043_calendar_mirror`、`git diff --check` 全部通过。附：修复 `test_chat_safety_api_lists_and_acks_alerts` 既有 flaky（FakeTime 下后台升级链与 HTTP ack 竞态，2/3 概率失败并可连锁挂起全量套件；改为断言前 `service.stop()`，6/6 稳定）。

- 2026-09-15 主动投递解耦修复：main.py 等价重构（无新用例）；mypy `app/main.py` 零错误，非 soak 全量 **877 通过 / 2 跳过**，本地运行 Hub 热重载后 healthz 正常。同日 SenseAudio 真实联调：音色目录/试听合成（Free + 克隆音色）经 Admin API 实测通过。
...
- 2026-09-06 `PC-02` 执行可视化后端：新增 `tests/test_plan_execution_control.py` **5 通过**（执行中取消：在途步骤如实保留+剩余记 cancelled+第二步 runner 从未被调用即迟到结果不推进；在途失败+取消意图收敛为 cancelled 而非 failed；事件序列 execution.started→step.started→step.finished×2→execution.finished 且含 completed/total 与验证证据状态；监听器异常不中断执行；API 取消 executing 计划返回 cancel_requested=true 并透传 reason）；既有 `test_action_plan.py` 16 例无回归；非 soak 全量 pytest **724 通过 / 0 失败**（1 个 soak 用例 deselect）；Ruff、严格 mypy（304 source files）、`git diff --check` 通过；Alembic SQLite 空库升级到单 head `0035_plan_cancel`。Chat 前端进度面板已随本批落地（PlanProgressCard + plan.execution 帧消费 + 停止按钮）；真机验收待做。
- 2026-09-06 `FLOW-01` 可复用流程：新增 `tests/test_workflows.py` **10 通过**（预览编译并附动作标签/风险/确认策略、未知动作/参数漂移/空步骤拒绝（注册表异常归一 ValueError）、保存/列表/删除与用户内同名（含大小写变体）拒绝/用户隔离/删除后可重建、运行展开为待确认计划（标题/参数落位）与显式 key 幂等、保存工具先预览确认且预览不落库、确认后保存同 turn 幂等、L2/未知动作拒绝、运行工具按 ID/名称查找且同 turn 幂等返回同一计划/未找到/缺目标拒绝、步骤模型拒额外字段、API 401/200/422/201/运行/404 全链）；非 soak 全量 pytest **719 通过 / 0 失败**（1 个 soak 用例 deselect）；Ruff、严格 mypy（303 source files）、`git diff --check` 通过；Alembic SQLite 空库升级到单 head `0034_workflows`。真实模型端的流程保存/运行体验待真机验收。
- 2026-09-06 `WEB-01` 浏览器工作流：新增 `tests/test_browser_form.py` **8 通过**（总开关关闭四工具全拒、open_tab 非 http/https scheme 拒绝、form.read/fill 能力要求与参数边界、fill 计划 ref 模式/空值/越界拒绝、submit 命令载荷、离线设备/失败回执 reason 透传/缺幂等键、Registry 四动作 A0/A1/A2 契约、聊天挂载恒 False）；`test_action_registry.py` catalog 扩展到 18 动作并把回执验证断言统一为「非 home 类全部 receipt」；扩展 `core.test.ts` 新增 4 例（顺序 ref 与密码掩码、控件数上限、fill 计划严格校验、fill 结果净化）共 **8 通过**；非 soak 全量 pytest **709 通过 / 0 失败**（1 个 soak 用例 deselect）；Ruff、严格 mypy（296 source files）、扩展 `tsc + vite` build、`git diff --check` 全部通过；无新迁移。真实 Chrome 端到端（读取/填写/受控组件页面/submit 确认流）待验收。
- 2026-09-06 `PC-01` 白名单桌面动作：新增 `tests/test_desktop_actions.py` **11 通过**（配置 casefold 去重/scheme 模式拒绝、split_url 边界、应用白名单未命中与禁用拒绝、URL scheme/主机白名单、音量与剪贴板开关、幂等键/用户/离线设备守卫、失败回执 reason 透传、Registry 四动作注册与 A1/A2 契约、L2 经 EgressGuard 拦截、聊天挂载恒 False）；`test_action_registry.py` catalog 断言扩展并把验证策略断言按动作类别收窄（home=read_after_write、desktop/system=receipt）；Desktop `protocol.test.ts` 新增 PC-01 参数校验 1 例（注入字符/越界/非法 scheme 全拒）共 **21 通过**；非 soak 全量 pytest **701 通过 / 0 失败**（1 个 soak 用例 deselect）；Ruff、严格 mypy（294 source files）、Desktop `tsc + vite` build、`cargo check`、`git diff --check` 全部通过；无新迁移。真实设备端到端（真机打开应用/URL/音量/剪贴板 + Action Plan 确认流）待验收。
