...
- [ ] **当前功能：Tauri 透明桌宠。** 透明置顶窗口、拖拽、鼠标穿透、位置恢复、托盘解锁、Hub 同源形象舞台及正式签名事件通道已完成；下一步做 macOS 多屏、CPU/内存与长时间运行真机验收。M2 仍作为发布门槛，不阻塞代码开发。
...
- [ ] P6 Batch D Tauri 桌宠：透明置顶窗口、点击穿透、拖拽、位置恢复、Hub 同源舞台及 `avatar.render` 签名临时事件已完成；待完成 macOS 多屏、CPU/内存与长时间运行验收。M2 首音频 P90 ≤1.8s、打断 P90 ≤300ms 保留为发布门槛。
