...
当前开发阶段，以下接口暂时复用 `ARIA_ADMIN_TOKEN` Bearer 鉴权：
...
- 尚未实现 WebSocket 流式 delta、取消和断线补拉；
- 尚未实现独立用户登录、短期 token、会话归属校验和聊天/管理分权；
- 尚未实现结构化 `AgentReply`、人格版本、工具候选及状态机；
- 最近 20 条只是工作上下文，不是四层记忆；记忆提取、溯源、冲突、删除验证和向量检索仍待实现；
- 同一会话的并发发送还需要显式回合状态与 generation 隔离。

下一步优先实现独立身份与 WebSocket 回合状态，再接入结构化回复和可删除的短期/情景记忆，避免把临时调试鉴权扩散成正式安全边界。
