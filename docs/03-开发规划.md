...
> 文档版本：v2.0（2026-08-29）
> 上游文档：[01-需求分析.md](./01-需求分析.md) | [02-功能设计.md](./02-功能设计.md)

> 当前实施进度：M1 文字核心、P5 14 天稳定性观察、P6 Batch A～C、M3A 软件底座与 M3B 认知闭环 v1 已落地；桌宠联动代码已完成但保留真机发布门槛。2026-08-29 起主线切换到手机 PWA，Batch A 可安装壳与移动布局已落地，下一步是 iOS/Android 真机聊天、权限和前后台重连验收。详细执行队列只看 [TASKS.md](./TASKS.md)。
...
**2026-08-29 当前状态**：P5 14 天文字稳定性观察已完成，P6 Batch A～C、Live2D Web 壳、Tauri 透明桌宠联动与系统内容选择器均已落地。用户决定将手机端提前为当前功能主线：PWA Batch A 已复用现有 Vue Chat 完成可安装壳、离线降级、窄屏会话/形象抽屉、刘海屏安全区和安装模式会话级令牌存储。下一步是 iOS Safari 与 Android Chrome 真机的安装、登录、文字/streaming、麦克风/定位权限和前后台 WebSocket 重连验收，随后再接手机通知与多端会话漫游。桌宠性能与 M2 延迟仍作为并行发布门槛。可变的测试数字统一维护在 `TASKS.md`。
...
4. ☐ 在 iPhone Safari 添加到主屏，验收登录、文字流式聊天、语音与定位权限。
5. ☐ 在 Android Chrome 验收安装提示、前后台切换、WebSocket 重连与离线降级。
