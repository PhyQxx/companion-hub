...
> 文档版本：v1.6（2026-08-20）
> 上游文档：[01-需求分析.md](./01-需求分析.md) | [02-功能设计.md](./02-功能设计.md)

> 当前实施进度：M1 文字核心与 M1.5 稳定性闸门可开发项已全部收口。多主体长期记忆（docs/30）Batch A～F 完成：后端回归、Vue 管理页、浏览器验收、真实模型 20 组用例通过。Timeline/History Recall（docs/31）第一版已落地：后端最小链路、Vue 管理页、浏览器真实数据验收通过。P5 于 2026-08-20 进入 14 天真实文字使用（与 P6 语音开发并行）。P6 Batch A（docs/33）语音闭环垂直切片已落地：/ws/voice、VAD、MiMo ASR/TTS + edge-tts 故障转移链、句级流式合成、barge-in 打断、语音配置中心化，全部通过自动化验收。下一步推进 P6 Batch B（前端麦克风 UI、本地 ASR、faster-whisper/silero-vad）。详见 [TASKS.md](../TASKS.md)。
...
**2026-08-20 当前状态**：P5 可开发项已全部完成并通过验收。多主体 Memory 和 Timeline/History Recall 的前后端、浏览器联调和真实模型回归均已收口。期间修复了 `_assistant_profile_overrides` 隐私泄漏漏洞（L2 档案进入 L1 上下文）和 Qwen3.6 reasoning 输出预算不足问题（`reasoning_overhead_tokens` + 120s 超时）。确定性记忆评估集 `memory_eval` 正例 20/20、负例 0 误引、冲突 8/8、删除 4/4、敏感隔离 4/4。P5 于 2026-08-20 起计 14 天真实文字使用，与 P6 语音开发并行。P6 Batch A 语音闭环已完成自动化与浏览器验收（含语音配置中心化），当前推进 Batch B（前端麦克风 UI、本地 ASR）。
