...
> 文档版本：v1.5（2026-08-18）
> 上游文档：[01-需求分析.md](./01-需求分析.md) | [02-功能设计.md](./02-功能设计.md)

> 当前实施进度：M1 文字核心已完成 Vue chat/admin、Persona、Memory v1、删除闭环与 pgvector 基线；现处于 M1.5 / P5 文字稳定性闸门。多主体长期记忆（docs/30）已完成 Batch A～D 后端，Batch E Vue 管理代码已落地但待 Node 环境 typecheck/build；Timeline/History Recall（docs/31）Phase A/B 后端最小闭环也已完成。下一步集中补真实模型 Batch F、Timeline/Memory 管理前端与浏览器联调，之后进入 14 天真实使用。详见 [29-P5前置联调与配置运行时收口.md](./29-P5前置联调与配置运行时收口.md)。
...
| LLM 接入 | litellm（OpenAI 兼容统一层） | dialogue=GLM/DeepSeek/Qwen 任选，utility=便宜模型，private=Ollama |
| ASR | faster-whisper（本地流式） | 备选 FunASR；云端 API 备选 |
| 唤醒词 | openWakeWord | 模型可自训 |
| VAD | silero-vad | 打断检测复用其能量/语音概率 |
| TTS | edge-tts（M2 起步）→ GPT-SoVITS（M5 定制音色） | CosyVoice 备选 |
...
| 形象渲染 | pixi.js + pixi-live2d-display（M3B）；three + three-vrm（M4B） | 核心质量闸门通过后再加入形象层 |
...
| M2 | 语音化 | 8~10 周 | 唤醒说话、可打断、调试形象口型同步 | FR-I2/I3, O2 |
| M3A | 感知与主动（核心 v1） | 8~10 周 | 单存在传感器、规则式主动性、分寸闸门和基础运维 | FR-I4/I6, P1/P2, A1~A4 |
| M3B | 形象与体验增强（v1.1） | 12~16 周 | Live2D、形象中心、主题、完整后台、反思和敏感设备 | FR-I7, P3/P4, O3/O8~O12, S3/S9~S11 |
| M4A | 静态图动态化 | 8~12 周 | L0/L1 形象工厂、资产任务流、校正预览 | FR-O13 |
| M4B | 多终端与 3D | 8~10 周 | PWA、Tauri、VRM、多端同步与所有权 | FR-O4/O5/O14 |
...
冻结新功能，连续 14 天真实使用。记录错误引用、遗漏、冲突事实、删除残留、成本与 P90 延迟；只允许修复核心问题。闸门未通过不得进入语音、Live2D 或多传感器开发。

**2026-08-19 当前状态**：P5 前置联调的模型/Persona 运行时主链已经收口；真实对话发现的“助手自身事实跨会话漂移”和“普通历史细节不应全部晋升长期记忆”都已有后端实现回应。多主体 Memory 已打通数据模型、Admin API、多主体检索、完整回合沉淀、回声抑制、跨会话 exact fact、确定性槽位冲突和生成一致性 Guard；Timeline/History Recall 已完成 0011 索引/旧数据回填、message/event 持续索引、用户时区时间解析、有界搜索、Source Expansion、删除联动与 recall meta。P5 当前主要剩余前端构建/浏览器验收、真实模型回归和 14 天真实使用。
...
**目标**：只用一个存在传感器验证“她知道你回来并有分寸地主动回应”，不让形象工厂阻塞核心伴侣闭环。

| # | 任务 | 产出 | 预估 |
|---|---|---|---|
| 3A.1 | MQTT 设备接入：注册、心跳、服务端隐私定级、遥测去抖 | 设备网关 | 3 天 |
| 3A.2 | ESP32 + LD2410 存在雷达节点与烧录文档 | `firmware/` | 2 天 |
| 3A.3 | 感知规则、用户模式合并和环境上下文 | `perception/` | 4 天 |
| 3A.4 | 主动引擎：触发、免打扰、冷却、上限和日志 | `proactive/` | 5 天 |
| 3A.5 | 输出仲裁与多端音频租约基础 | 输出网关增强 | 3 天 |
| 3A.6 | 单传感器 7 天分寸闸门和隐私攻击测试 | 验收报告 | 7+3 天 |
| 3A.7 | 基础后台：设备、主动日志、服务健康、告警和恢复演练 | `web/admin` | 5 天 |
| 3A.8 | 核心 v1 全项验收与修缺 | 验收报告 | 5 天 |
...
| # | 任务 | 产出 | 预估 |
|---|---|---|---|
| 3B.1 | Live2D 渲染端：情绪、动作、口型和能力降级 | `web/avatar-live2d` | 5 天 |
...
**部署主机**：家中 7×24 设备，最低 4 核 8G（无 GPU 跑云端模型 + CPU ASR 可用但 ASR 延迟高；有 NVIDIA 显卡（≥8G 显存）则本地 whisper/GPT-SoVITS/Ollama 全家桶体验最佳）。
