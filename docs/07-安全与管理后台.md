...
| 伴侣核心 | 人格与表达 | 基本设定、表达风格、边界与主动、动作映射、版本历史 | `viewer`；修改需 `admin` |
| 伴侣核心 | 形象与外观 | 形象库、主题中心 | `viewer`；修改需 `admin` |
| 伴侣核心 | 记忆与历史 | 记忆库、质量与冲突、历史时间线、删除验证 | `admin` |
| 伴侣核心 | 个人管家 | 任务、流程、场景、会议、简报与回顾 | `admin` |
| 能力接入 | 模型与路由 | 模型服务、路由与能力、工具与动作、语音服务、声音管理、MCP 工具 | `viewer`；修改需 `admin` |
| 能力接入 | 集成与连接 | 邮件、外部日历、小爱音箱 | `admin` |
| 能力接入 | 设备终端 | 设备与配对、命令记录、健康诊断 | `operator`；授权变更需 `admin` |
| 能力接入 | 感知与守护 | HA 实体授权（含连接配置）、屏幕感知、浏览感知、安全守护 | `operator`；授权变更需 `admin` |
| 能力接入 | 主动输出 | 输出通道（单工作区模块，不渲染 Tab 栏） | `admin` |
...
**2026-09-22 信息架构重整第二批**：原"设备与感知"（11 个 Tab）拆分为「设备终端」（设备与配对/命令记录/健康诊断）、「感知与守护」（HA 实体授权/屏幕感知/浏览感知/安全守护）与「主动输出」（输出通道）三个模块；屏幕感知与浏览感知各自的"运行状态/观察记录"合并为单 Tab 内 `section` 切换（URL `?section=records` 持久化）；"配对授权"独立页删除（设备列表页已有行级配对码与"生成配对码"弹窗，原页面完全冗余）；单工作区模块（提醒与任务、主动输出）不再渲染 Tab 栏。HA 连接配置（地址/令牌/超时/重连）从"模型与路由"迁入"感知与守护 → HA 实体授权"页顶部连接卡片，模型页不再持有 `integrations` 草稿（保存经 `mergePreservingUnknown` 合并，不触碰服务端 HA 配置）。新增「集成与连接」模块：邮件（MAIL-01 SMTP/IMAP，此前无任何 UI）、外部日历（CAL-01，自 HA 实体授权页迁出）、小爱音箱（自 HA 实体授权页迁出，音箱下拉复用 HA 设备缓存的 localStorage 键）。"模型与路由 → 工具服务"更名"工具与动作"，新增桌面动作白名单（PC-01）、浏览器工作流开关（WEB-01）与出行管家（COMMUTE-01）配置卡片——PC-01 白名单此前只能经 API 修改。人格"主动发起话题"并入"边界与主动"Tab；记忆"质量分析/冲突处理"合并为"质量与冲突"（页内切换）。旧深链兼容：`/devices?tab=…`、`/models?tab=home_assistant`、`/memory?tab=conflicts`、`/personas?tab=proactive` 经路由守卫迁移到新模块对应位置（含 `section` 意图保留）。
...
> 最后更新：2026-09-22（新增 Admin 备份状态卡片与数据导出/导入）
> 对应需求：FR-S3（数据库定时备份）。备份只保护数据库与配置卷；产品级
> JSON 导出/导入已于 2026-09-22 交付（见下节「数据导出与导入」）。

**Admin 可视化（2026-09-22）**：「系统与维护 → 存储与备份」顶部新增备份状态卡片，数据来自 `GET /api/v1/admin/backups`（Admin Token 鉴权）——列出 `aria_*.dump` 转储（名称/大小/时间，最新在前，上限 30 条）、回显 `ARIA_BACKUP_KEEP_DAYS/ARIA_BACKUP_AT/ARIA_BACKUP_TZ` 配置，并按「最近备份是否超过 26 小时（每日节奏 + 补跑宽限）」给出健康徽标；目录缺失或为空同样判不健康并给自检提示（检查 backup profile 是否启用、`./backups:/backups:ro` 挂载是否就位）。Hub 对备份目录**只读**（compose `:ro` 挂载 + `ARIA_BACKUP_DIR=/backups`；本地开发默认相对 `backups/`），转储与滚动清理由 backup profile 容器独占执行，Admin 不提供删除或触发备份入口。

### 1. 边界与不变量

- 备份内容为 Postgres 逻辑备份（`pg_dump --format=custom`），覆盖事件、
  记忆、时间线、任务、日历、联系人、行动计划、配置中心等全部业务表；
- 备份与数据库使用同一 `pgvector/pgvector:pg16` 镜像，`pg_dump` 版本
  与服务端严格一致，避免跨版本恢复问题；
- 备份产物写入宿主机 `./backups/`（已 gitignore），不进任何 git 或
  对象存储；文件包含全部记忆与私密内容，**主机级安全等同于数据库卷**；
- 每日一次（默认 03:30，时区由 `ARIA_BACKUP_TZ` 控制，默认 Asia/Shanghai），
  保留天数默认 14，按文件 mtime 滚动删除；失败自动重试（默认 10 分钟间隔），
  容器启动晚于当日时刻时补跑一次，保证部署即有备份。

### 2. 启用

```bash
# .env 追加（可选，均有默认值）
ARIA_BACKUP_AT=03:30
ARIA_BACKUP_KEEP_DAYS=14

docker compose --profile backup up -d
docker compose logs -f backup   # 首次启动数秒内应出现一条 wrote 日志
```

### 3. 恢复步骤（隔离演练后才允许写入生产卷）

```bash
# 1) 停止写入方（保留备份容器与 postgres 容器运行）
docker compose stop hub

# 2) 空库恢复演练（不触碰 postgres-data 卷）：
docker run --rm --network companion-hub_default -v "$PWD/backups:/backups:ro" \
  -e PGPASSWORD="$(grep '^POSTGRES_PASSWORD' .env | cut -d= -f2)" \
  pgvector/pgvector:pg16 \
  sh -c 'createdb --host=postgres --username=aria aria_drill &&
         pg_restore --host=postgres --username=aria --dbname=aria_drill
                    --no-owner /backups/aria_YYYYMMDD_HHMMSS.dump'
# 核对 aria_drill 关键表行数（message / timeline_event / task_item /
# calendar_event / config_version）与生产一致后删除演练库。

# 3) 真实恢复（危险：整卷回滚到备份时刻，其后数据全部丢弃）：
docker compose down   # 同时停掉 postgres，避免继续写入
docker volume rm companion-hub_postgres-data
docker volume create companion-hub_postgres-data
docker compose up -d postgres
# 待 postgres 健康后：
docker run --rm --network companion-hub_default -v "$PWD/backups:/backups:ro" \
  -e PGPASSWORD="$(grep '^POSTGRES_PASSWORD' .env | cut -d= -f2)" \
  pgvector/pgvector:pg16 \
  pg_restore --host=postgres --username=aria --dbname=aria \
             --no-owner /backups/aria_YYYYMMDD_HHMMSS.dump
docker compose up -d    # 重启 hub；alembic 版本随备份回到历史值，直接启动即可
```

### 4. 验收清单

- [ ] 备份容器上线后连续 3 天每天产出一份 `.dump`，失败有日志；
- [ ] 保留策略生效：手工放置 15 天前的假文件（touch -d）下一轮被清理；
- [ ] 空库恢复演练（aria_drill）成功，关键表行数与生产核对一致；
- [ ] 恢复演练后删除演练库，生产库零改动。

## 数据导出与导入（FR-S3，2026-09-22）

> Admin「系统与维护 → 存储与备份」的数据导出卡片；API 为
> `GET /api/v1/admin/data/export` 与 `POST /api/v1/admin/data/import`
> （Admin Token 鉴权）。

### 1. 边界与不变量

- **只导出伴侣数据**：用户、人格版本、对话、消息、记忆（含来源）、时间线、
  任务、承诺、联系人、日历、简报/回顾、会议、流程与家庭场景；导出为单一
  JSON 档案（`format=aria-export`、`schema_version=1`、hub 版本与时间戳），
  字段按真实表列驱动（属性名与列名解耦，兼容 `metadata_json`→`metadata`
  这类重命名列）；
- **凭据与机器状态绝不导出**：登录凭据/会话、设备注册与命令、推送订阅、
  OAuth 刷新令牌、配置中心（含 secret）、运行时租约、任务引擎、资产库、
  审计类日志（认知决策/反馈/语义事件审计/删除台账）均不在档案中；
- **导入只支持恢复到空库**：目标库的 message/memory/task_item 任一有数据
  即 409 拒绝；必须显式 `confirm=true`；全程单事务，失败整体回滚；
  按外键依赖顺序插入（用户 → 人格 → 对话 → 消息 → 记忆 → 来源 → …）；
- **导入不触发副作用**：不自动发布人格、不发投递、不写任何外部系统；
- 导出文件包含全部 L2 私密内容，**敏感度等同数据库备份**——主机级安全、
  不进 git/对象存储，传输仅经 Admin Token 鉴权的 HTTPS/本地通道。

### 2. 使用方式

- 导出：Admin 存储与备份 Tab → 「导出 JSON 档案」，浏览器下载
  `aria-export-YYYYMMDD-HHmm.json`；
- 导入：同卡片「选择档案导入…」→ 确认对话框 → 返回逐表插入计数；
  换机流程建议：新机装好 Hub 并完成迁移后，先导入档案再登录使用
  （登录凭据在新机重新设置，不随档案迁移）。
