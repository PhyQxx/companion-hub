...
### 1.1 当前运行数据库快照（2026-08-20，config v23）

本机 `GET /api/v1/admin/config/current` 的非敏感摘要显示：

- `dialogue`: `sensenova_dialogue` → `local_private` → `sensenova_utility` → `glm-4.7-flash` → `glm-5.3`，route timeout 为 12s；
- `utility`: `sensenova_utility` → `sensenova_dialogue` → `glm-5.3` → `glm-4.7-flash`；
...
- 另有 `sensenova_reasoning_fallback=glm-5.2` 端点存在，但当前 v23 三条文本路由没有引用它；
...
- LLM Router 的 429 / route timeout 采用快速故障转移：不在同一 endpoint 上重复消耗一次完整重试预算，直接进入下一个 fallback。当前 LM Studio Qwen 本地生成探针返回 200，可作为云端拥堵时的本地兜底。
...
```text
L0/L1 ── dialogue ──> 当前 database dialogue 路由（v16: deepseek → utility → glm-4.7 → glm-5.3）
      └─ utility  ──> 当前 database utility 路由（v16: utility → deepseek → glm-5.3 → glm-4.7）
