...
> 状态：已实现并纳入 CI。配置基线日期：2026-08-19。

## 1. 决策摘要

Companion Hub 不把业务逻辑绑定到某家模型。认知层只依赖 `CompletionRequest / CompletionResult`，供应商由 OpenAI-compatible 适配器封装，模型选择由数据库中的已发布配置决定。

当前采用“商汤承担常规文本主链、智谱免费模型补齐文本与多模态能力、私密数据只走本地”的组合。文本路由与视觉/生成能力分离，避免把生图或视频模型误接到聊天 fallback：

| 场景 | 默认模型 | 备用模型 | 原因 |
|---|---|---|---|
| 日常陪伴对话、短问答、情绪化表达 | 商汤 `deepseek-v4-flash` | 商汤通道 `glm-5.2` → 智谱 `glm-4.7-flash` | 保留既有稳定主链，免费 GLM 作为额外容灾 |
| 分类、摘要、记忆提取、标签生成、意图判断 | 商汤 `sensenova-6.8-flash-lite` | `deepseek-v4-flash` → 智谱 `glm-4.7-flash` | 高频后台任务优先速度，GLM 只在前序失败后接管 |
...
```text
L0/L1 ── dialogue ──> deepseek-v4-flash ──失败──> glm-5.2 ──失败──> glm-4.7-flash
      └─ utility  ──> sensenova-6.8-flash-lite ──失败──> deepseek-v4-flash ──失败──> glm-4.7-flash
