...
export class LAppModel extends CubismUserModel {
  ...
  public update(): void {
    ...
    if (this._externalLipSyncEnabled || this._externalLipSyncValue > 0.001) {
      const blend = Math.min(1.0, deltaTimeSeconds * 22.0);
      this._externalLipSyncValue +=
        (this._externalLipSyncTarget - this._externalLipSyncValue) * blend;
      for (const id of this._lipSyncIds) {
        this._model.setParameterValueById(id, this._externalLipSyncValue);
      }
      if (!this._externalLipSyncEnabled && this._externalLipSyncValue < 0.001) {
  ...
  /** Returns true after the model, textures and shaders are ready to render. */
  public isReady(): boolean {
    return this._state === LoadStep.CompleteSetup;
  }

  public setExternalLipSync(value: number): void {
    this._externalLipSyncEnabled = true;
    this._externalLipSyncTarget = Math.max(0, Math.min(1, value));
  }

  public setSpeaking(active: boolean): void {
    if (active) {
      this.startFirstAvailableMotion(['Tap', 'Tap@Body', 'TapBody']);
      return;
    }
    this._externalLipSyncEnabled = false;
    this._externalLipSyncTarget = 0;
  }

  public setExpressionIfAvailable(expressionId: string): boolean {
    const normalized = this.normalizeControlName(expressionId);
    const match = [...this._expressions.keys()].find(key => {
      const candidate = this.normalizeControlName(key);
      return candidate === normalized || candidate.includes(normalized);
    });
    if (!match) return false;
    this.setExpression(match);
    return true;
  }
  ...
  public setEmotion(emotion: string): void {
    const expressions: Record<string, string[]> = {
      ...
      concerned: ['concerned', 'worry', 'worried']
    };
    for (const expression of expressions[emotion] ?? [emotion]) {
      if (this.setExpressionIfAvailable(expression)) break;
    }
    ...
    const motions: Record<string, string[]> = {
      ...
      surprised: ['FlickUp', 'Tap'],
      thinking: ['Flick'],
      concerned: ['FlickDown']
    };
    this.startFirstAvailableMotion(motions[emotion] ?? []);
  }

  public playMotionGroup(group: string, index?: number): boolean {
    const requested = this.normalizeControlName(group);
    let selected = '';
    for (let i = 0; i < this._modelSetting.getMotionGroupCount(); i++) {
      const candidate = this._modelSetting.getMotionGroupName(i);
      if (this.normalizeControlName(candidate) === requested) {
        selected = candidate;
        break;
      }
    }
    const count = selected ? this._modelSetting.getMotionCount(selected) : 0;
    if (count === 0) return false;
    const motionIndex = index == null
      ? Math.floor(Math.random() * count)
      : Math.max(0, Math.min(count - 1, index));
    this.startMotion(selected, motionIndex, LAppDefine.PriorityForce);
    return true;
  }

  private startFirstAvailableMotion(groups: string[]): boolean {
    for (const group of groups) {
      if (this.playMotionGroup(group)) return true;
    }
    return false;
  }

  private normalizeControlName(value: string): string {
  ...
  private _externalLipSyncEnabled: boolean;
  private _externalLipSyncTarget: number;
